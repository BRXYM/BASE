...
import {
  ...
  addSela, deleteSela, getAllSelas, getSelaById, updateSela
} from '@/api/api';
...
export const useGoodStore = defineStore('good', {
  ...
  actions: {
     ...
     * @param good - 商品对象
     */
    async addGood(good: Partial<Good>) {
      const { data: { statusCode, code, list, message } } = await addGood(good);
      if (statusCode === 200) {
        ElMessage.success("添加成功");
...
export const useClaStore = defineStore('cla', {
  ...
  actions: {
     ...
     * @param cla - 课程对象
     */
    async addCla(cla: Partial<Cla>) {
      const { data: { statusCode, code, list, message } } = await addCla(cla);
      if (statusCode === 200) {
        ElMessage.success("添加成功");
